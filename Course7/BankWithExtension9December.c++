...
void GoBackToMainMenueForUsers(vector<stUN> &vUsers, vector<CD> &vClients, stUserName &UN);
void GoBackToManageMenueForUsers(vector<stUN> &vUsers, vector<CD> &vClients, stUserName &UN);
...
void PerformMainMenueOptionForUsers(vector<stUN> &vUsers, vector<CD> &vClients, stUserName &UN);
void PerformManageUsersMenueScreen(vector<stUN> &vUsers, vector<CD> &vClients, stUserName &UN);
...
void EnterUserNameAndPassword(vector<stUN> &vUsers, vector<CD> &vClients)
{
    ...
    while (true)
    {
        ...
        if (FindUserByUserNameAndPassword(vUsers, UN))
            PerformMainMenueOptionForUsers(vUsers, vClients, UN);
...
void GoBackToManageMenueForUsers(vector<stUN> &vUsers, vector<CD> &vClients, stUserName &UN)
{
    cout << "\n\nPress any key to back to main menue...";
    system("pause > 0");
    PerformManageUsersMenueScreen(vUsers, vClients, UN);
}
void GoBackToTransactionsMenue(vector<CD> &vClients)
{
...
void PerformMainMenueOptionForUsers(vector<stUN> &vUsers, vector<CD> &vClients, stUserName &UN)
{
    ...
    case enMainMenueForUsers::ShowClientList:
        ...
        {
            ShowAccessIsDenied();            
            GoBackToMainMenueForUsers(vUsers, vClients, UN);
        }
...
void PerformManageUsersMenueScreen(vector<stUN> &vUsers, vector<CD> &vClients, stUserName &UN)
{
    ...
    case enManageUsersMenue::UsersList:
        ...
        {
            ShowVectorIsEmpty();
            GoBackToManageMenueForUsers(vUsers, vClients, UN);
        }
