...
int NumberOfDaysInMonth(stDate Date, short month);
...
{	
	...
	cout << "\nYour Age In Days is: " << GetDifferenceInDays(Date2, Date1);
}
...
int DaysFromBeggingOfYear(stDate Date)
{
	...
	{
		numberOfDaysInAMonth += NumberOfDaysInMonth(Date, i);
	}
...
int NumberOfDaysInMonth(stDate Date, short month)
{
	return((isLeapYear(Date.year) && month == 2) ? 29 : NumberOfDaysInMonthInNotLeapYear(Date));
}
...
string ConvertTotalDaysToDate(int year, int daysFromBeggingOfYear)
{
	...
	while (true)
	{
		numberOfDaysInMonth = NumberOfDaysInMonth(Date, Date.month);
...
{
	signed int differenceInDays = 0;
	differenceInDays -= (NumberOfDaysInAYear(Date1.year) - DaysFromBeggingOfYear(Date1));
    differenceInDays -= DaysFromBeggingOfYear(Date2);
	...
	cout << "\n\n\ndays after subtracting the number of days in "<< to_string(Date1.year) << " is " << (NumberOfDaysInAYear(Date1.year) - DaysFromBeggingOfYear(Date1));
