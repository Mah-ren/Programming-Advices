...
int main()
{
    ...
    IsDateWithinPeriod(Period, Date) ? cout << "\nYes, Date is within Period." : cout << "\nYes, Date is not within Period.";
...
bool IsDateWithinPeriod(stPeriod Period, stDate Date)
{
    IsDate1BeforeDate2(Period.start, Date) && IsDate1BeforeDate2(Date, Period.end) ? true : (IsDate1EqualToDate2(Period.start, Date) || IsDate1EqualToDate2(Period.end, Date)) ? true : false; 
}
